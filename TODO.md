...
- [x] Multi-window: **live cross-window sync** — a shared `DocSignal` (gpui global, one per process) is emitted on every content save (the `save_page_content` choke point); other windows reload the changed journal days / active page from the DB (`apply_external_edit`), via value-comparison so only stale content is touched and the editing window never clobbers itself. Remaining: same-page concurrent *edits* are still last-write-wins, and backlinks / the sidebar page-list aren't live-synced yet (need a signal from rename/create/delete)
