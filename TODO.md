...
- [x] Sidebar: **horizontal scroll on overflow** — a deep namespace path or a long page title wider than the rail scrolls sideways instead of being clipped or wrapping: the tree rows are `whitespace_nowrap` and the list is `overflow_scroll` + `items_start` (so it sizes to its widest row). See `src/ui/sidebar.rs`
